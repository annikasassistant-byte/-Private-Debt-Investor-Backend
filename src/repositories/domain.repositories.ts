...
export class PaymentRepository extends BaseRepository {
  ...
  /** Expose model for bulk schedule ops used by InvestmentService. */
  get collection() {
    return this.model;
  }
  ...
  insertMany(docs: Record<string, unknown>[]) {
    return this.model.insertMany(docs);
  }

  updateMany(filter: Record<string, unknown>, update: Record<string, unknown>) {
    return this.model.updateMany(filter as any, update as any);
  }
}
