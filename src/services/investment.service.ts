...
export class InvestmentService {
  ...
  async list(query: Record<string, any> = {}, investorScopeId?: string | null) {
    ...
    return {
      data: result.data.map(mapInvestment),
  ...
  async generateSchedule(investmentId: string, _actorId?: string) {
    ...
    await (this.payments as any).model.deleteMany({ investment: investmentId });
    ...
    const docs = rows.map((row, idx) => {
      let status: string = PAYMENT_STATUS.SCHEDULED;
    ...
    if (docs.length) await (this.payments as any).model.insertMany(docs);
  ...
  async regenerateSchedule(investmentId: string, _actorId?: string) {
    ...
    const docs = rows.map((row) => {
      let status: string = PAYMENT_STATUS.FUTURE;
      if (row.dueDate < today) status = PAYMENT_STATUS.OVERDUE;
      else if (!upcomingSet) {
        status = PAYMENT_STATUS.UPCOMING;
        upcomingSet = true;
  ...
  async markPaid(paymentId: string, input: Record<string, any> = {}, actorId?: string) {
    ...
    const scheduledTotal = Number(payment.total) || 0;
    if (!(scheduledTotal > 0)) throw ApiError.badRequest('Payment has invalid total');

    // Cap amountPaid at payment.total (BUG-004)
    let amountPaid = Number(input.amountPaid ?? scheduledTotal);
    if (!(amountPaid > 0)) throw ApiError.badRequest('amountPaid must be positive');
    if (amountPaid > scheduledTotal) amountPaid = scheduledTotal;

    const scheduledPrincipal = Number(payment.principal) || 0;
    const scheduledInterest = Number(payment.interest) || 0;
    const isPartial = amountPaid < scheduledTotal;

    // Interest first, then principal — only prorated portions credit the investment
    const interestPortion = roundMoney(Math.min(amountPaid, scheduledInterest));
    const principalPortion = roundMoney(amountPaid - interestPortion);
    const status = isPartial ? PAYMENT_STATUS.PARTIALLY_PAID : PAYMENT_STATUS.COMPLETED;
    const paymentDate = input.paymentDate ? new Date(input.paymentDate) : new Date();

    const investment = await this.investments.findById(payment.investment);
    const priorOutstanding = Number(investment?.outstandingBalance ?? scheduledPrincipal);
    // outstanding = previousOutstanding - principalPortion (not payment.remainingBalance)
    const outstandingBalance = roundMoney(Math.max(0, priorOutstanding - principalPortion));
    ...
    await this.payments.update(paymentId, {
      ...
      status,
      remainingBalance: isPartial
        ? roundMoney(
            (Number(payment.remainingBalance) || 0) + scheduledPrincipal - principalPortion,
          )
        : Number(payment.remainingBalance) || 0,
    });

    if (investment) {
      const principalRepaid = roundMoney((investment.principalRepaid || 0) + principalPortion);
      const interestEarned = roundMoney((investment.interestEarned || 0) + interestPortion);
    ...
    await this.timeline.create({
      ...
      title: isPartial ? 'Partial Payment' : 'Payment Completed',
      description: isPartial
        ? `Payment #${payment.sequence} partially paid (${amountPaid} of ${scheduledTotal})`
        : `Payment #${payment.sequence} completed`,
      date: paymentDate,
      amount: amountPaid,
      status: 'completed',
    ...
    await this.audit?.log({
      ...
      meta: { amountPaid, principalPortion, interestPortion, isPartial },
    });
  ...
  async markOverdue(investmentId?: string) {
    ...
    await (this.payments as any).model.updateMany(filter, {
  ...
  async refreshInvestmentPaymentMeta(investmentId: string) {
    ...
    if (next && next.status !== PAYMENT_STATUS.OVERDUE) {
      await (this.payments as any).model.updateMany(
        {
  ...
  async listTimeline(query: Record<string, any> = {}, investorScopeId?: string | null) {
    ...
    return {
      data: result.data.map(mapTimeline),
      meta: (result as any).meta || (result as any).pagination,
    };
  }
}
